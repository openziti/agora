...
Documentation has moved beyond the initial architectural sketch. The cross-cutting design decisions are captured in [foundation.md](foundation.md), and each of the six concepts has its own dedicated spec. Post-MVP extensions are tracked in the [roadmap cards](../../future/roadmap/) and per-concept specs: session multiplexing over a shared tunnel, programmatic contracts, envelope-level signing, richer A2A bridging, metrics, limits, and the other items collected there.
...
The [roadmap cards](../../future/roadmap/) and per-concept specs record the remaining post-MVP work.
...
Those are tracked in the [roadmap cards](../../future/roadmap/) and the per-concept specs' deferred sections.

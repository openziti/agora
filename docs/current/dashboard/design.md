...
- the post-MVP metrics and limits work tracked in [metrics](../../future/roadmap/metrics.md) and [resource limits](../../future/roadmap/resource-limits.md); the time-series substrate this work introduces is a precursor that accelerates that roadmap item but does not satisfy it
...
The implementation is intentionally narrow — exactly enough to drive the dashboard, no more. It is a precursor to the post-MVP metrics work tracked in [metrics](../../future/roadmap/metrics.md), not a substitute for it: the metrics work calls for ingest and query APIs, org-scoped visibility, automatic emission, and TimescaleDB-backed storage. The dashboard's substrate uses plain PostgreSQL, in-process emission, and a single read path.
...
- the post-MVP metrics work tracked in [metrics](../../future/roadmap/metrics.md) (this work is a precursor that accelerates that roadmap item but does not replace it)

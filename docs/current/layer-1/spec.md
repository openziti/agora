...
HTTP proxy tunnels preserve streaming response behavior. When an origin flushes a response, including headers sent before the first body byte, the provider and consumer proxies make that response visible to the client without waiting for body data.

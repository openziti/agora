...
**A host goes half-dead.** One afternoon the server is wedged — still connected, not actually serving — so it's still claiming the spot and Alice's laptop can't simply take over. She serves with `--takeover`: it kicks off whatever is currently hosting the tunnel — the wedged server — and her laptop takes the spot. Takeover is deliberate and blunt: it displaces whatever it finds, so Alice points it at the right tunnel and means it.
...
The controller recognizes a single current host at a time — for a managed proxy tunnel that is the serve record; for a direct tunnel it is simply whichever environment has the listener open. Moving the host to another of the account's environments is ordinary: the new environment serves, and when the previous host exited cleanly there is nothing in the way. When the previous host died *un*cleanly and still appears to be hosting, `--takeover` clears the way — it kicks off whatever is currently hosting the tunnel, evicting what that host left behind on the fabric (and clearing the serve record, for a proxy tunnel), so the new host can take the spot. Takeover is blunt by intent: it displaces whatever it finds rather than trying to judge whether that host is healthy. A direct tunnel has no record or heartbeat to judge with anyway, and rather than make the two shapes behave differently, takeover means the same thing for both — take it now.
...
- Any environment of the owning account may host the tunnel; no other account's environments may. Who is hosting *now* is shape-dependent: a proxy tunnel records it (the serve record); a direct tunnel does not — its host is observable only as the live listener on the fabric.
- `--takeover` is a controller-driven fabric eviction, not a bookkeeping edit: it finds the tunnel's current host (the serve record for a proxy tunnel; the live terminators on the fabric for a direct tunnel) and removes it — deleting its terminators, and clearing the serve record for a proxy tunnel. This is the one-shot, manual slice of terminator management; the continuous version lives in [single-active-listen](./single-active-listen.md).
...
- The controller recognizes one current host at a time (the serve record for a proxy tunnel; the open listener for a direct tunnel). `--takeover` lets an eligible environment reclaim the spot by kicking off whatever is currently hosting — evicting what it left on the fabric, and clearing the serve record for a proxy tunnel. Takeover is unconditional: it displaces whatever it finds and does not check the current host's health (a direct tunnel has nothing to check).

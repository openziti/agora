...
func main() {
	...
	if err := app.Run(func(ctx context.Context, a *agent.Agent) error {
		...
		handler := &agentutil.RequestResponseHandler[payloads.SearchRequest, payloads.SearchResponse]{
			Agent:               a,
			ResponseMessageType: "signals.search.response",
			Handle:              agentutil.SearchHandleFor(a, a.Live()),
		}
		go func() {

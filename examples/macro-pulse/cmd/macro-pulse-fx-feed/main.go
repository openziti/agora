...
func main() {
	...
	if err := app.Run(func(ctx context.Context, a *agent.Agent) error {
		...
		handler := &agentutil.RequestResponseHandler[payloads.FXRequest, payloads.FXResponse]{
			Agent:               a,
			ResponseMessageType: "markets.fx.response",
			Handle:              agentutil.FXHandleFor(a, a.Live()),
		}
		go func() {

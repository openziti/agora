...
func main() {
	...
	if err := app.Run(func(ctx context.Context, a *agent.Agent) error {
		...
		handler := &agentutil.RequestResponseHandler[payloads.NarrateRequest, payloads.NarrateResponse]{
			Agent:               a,
			ResponseMessageType: "analytics.narrate.response",
			Handle:              agentutil.NarrateHandle,
		}
		go func() {

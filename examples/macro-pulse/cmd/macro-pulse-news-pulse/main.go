...
func main() {
	...
	if err := app.Run(func(ctx context.Context, a *agent.Agent) error {
		...
		handler := &agentutil.RequestResponseHandler[payloads.NewsRequest, payloads.NewsResponse]{
			Agent:               a,
			ResponseMessageType: "signals.news.response",
			Handle:              agentutil.NewsHandleFor(a, a.Live()),
		}
		go func() {

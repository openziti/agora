...
func ServeTCP(ctx context.Context, factory OverlayFactory, identityPath, serviceName, backendTarget string) error {
	handle, err := StartServeTCP(ctx, factory, identityPath, serviceName, backendTarget)
	if err != nil {
		return err
	}
	return <-handle.Done()
...
func StartServeTCP(ctx context.Context, factory OverlayFactory, identityPath, serviceName, backendTarget string) (*Handle, error) {
...
func ConnectTCP(ctx context.Context, factory OverlayFactory, identityPath, serviceName, listenAddress string) error {
	handle, err := StartConnectTCP(ctx, factory, identityPath, serviceName, listenAddress)
	if err != nil {
		return err
	}
	return <-handle.Done()
...
func StartConnectTCP(ctx context.Context, factory OverlayFactory, identityPath, serviceName, listenAddress string) (*Handle, error) {
	overlay, err := factory.New(identityPath)
	if err != nil {
		return nil, err
	}
